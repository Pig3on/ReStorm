...
export function getWeatherAsyncAction(currentCity) {
  return async dispatch => {
    try {
      ...
      dispatch(
        weatherFailedAction(
          'There has been an error fetching data: ' + e.message,
        ),
      );
    }
  };
...
async function getWeatherByGps() {
  ...
  if (granted) {
    ...
    return await fetchWeatherByGps(
...
async function requestLocationPermission() {
  const granted = await PermissionsAndroid.request(
    PermissionsAndroid.PERMISSIONS.ACCESS_FINE_LOCATION,
    {
      ...
      message:
        'This application needs to access your location so it can get weather info based on your location',
